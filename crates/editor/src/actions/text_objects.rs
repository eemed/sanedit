...
fn select_range(editor: &mut Editor, id: ClientId, start: &str, end: &str, include: bool) {
    select_impl(editor, id, |slice, pos| {
        find_range(slice, pos, start, end, include)
    });
}

fn select_impl<F: Fn(&PieceTreeSlice, u64) -> Option<BufferRange>>(
    editor: &mut Editor,
    id: ClientId,
    f: F,
) {
